...
router.get("/adminacademicdb",async function(req,res) {
    if (req.isAuthenticated() && req.user.ROLE==="ADMIN"){
       connection.query(`SELECT P.PROJECT_ID,P.PROJECT_TITLE,S.NAME,S.USN,MAX(G.TOTAL_MARKS) AS MAX FROM PROJECT_DETAILS P,STUDENT_DETAILS S,GRADES G 
                         WHERE P.PROJECT_ID=G.PROJECT_ID AND G.USN=S.USN GROUP BY P.PROJECT_ID; `, function (err, result1) {
            if (err) {
                console.log(err);
            } else {
                connection.query(`SELECT P.PROJECT_ID,P.PROJECT_TITLE,AVG(G.TOTAL_MARKS) AS AVERAGE FROM PROJECT_DETAILS P,GRADES G 
                                    WHERE P.PROJECT_ID=G.PROJECT_ID GROUP BY P.PROJECT_ID;`, function (err, result2) {
                    if (err) {
                        console.log(err);
                    } else { 
                        connection.query(`SELECT S.USN,S.NAME,COUNT(*) AS COUNT FROM STUDENT_DETAILS S,STUD_PROJ_DETAILS P WHERE 
                            ...
                            if (err) {
                                ...
                                connection.query(`SELECT PROJECT_ID FROM PROJECT_DETAILS WHERE TEACHER_ID IS NULL;`, function (err, result4) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                                         console.log(result1);
                                         console.log(result2);
                                         console.log(result3);
                                         res.render("adminacademicdb",{result1:result1,result2:result2,result3:result3});
                                    }
                                });                   
                            }
                        });                    
